...
Em Outra Máquina (do Professor) 
Driver Linux LKM — Zero-Copy I/O e DMA 

� O QUE ESTE PROJETO FAZ: Implementa um driver de kernel Linux (LKM) que demonstra 
empiricamente as técnicas de Zero-Copy I/O via mmap() e Direct Memory Access (DMA) via 
dma_alloc_coherent + dma_mmap_coherent. Compara o desempenho do read() tradicional (com cópia) 
versus mmap() (sem cópia) em ambiente isolado de tempo real (SCHED_FIFO + mlockall + CPU pinning). 
...
•. Git 
⚠️ ATENÇÃO IMPORTANTE: Se for usar uma VM Azure, configure-a como 'Standard Security Type' (NÃO 
use 'Trusted Launch'). VMs com Trusted Launch têm Secure Boot ativo, que bloqueia o carregamento de 
módulos não assinados. Esta limitação está documentada no item 6.2 do README.md. 
...
# Atualiza pacotes já instalados (opcional, mas recomendado) 
ℹ️ Note: O upgrade pode demorar alguns minutos. Se aparecer alguma tela azul perguntando sobre 
reiniciar serviços, selecione 'Sim' (Tab → Enter). 
3.2 — Instalar ferramentas de compilação e cabeçalhos do kernel 
$ sudo apt-get install build-essential -y 
# Instala GCC, make e ferramentas para compilar código C 
$ sudo apt-get install linux-headers-$(uname -r) -y 
# Instala os cabeçalhos do kernel atual (necessário para compilar módulos) 
$ sudo apt-get install linux-modules-extra-$(uname -r) -y 
# Instala módulos extras (inclui drivers DMA opcionais) 
3.3 — Instalar Python e dependências para os gráficos 
$ sudo apt-get install python3-pip -y 
# Instala o gerenciador de pacotes Python 
$ pip3 install matplotlib pandas numpy 
# Instala as bibliotecas para gerar os gráficos 
⚠️ Atenção: Em distribuições mais novas (Ubuntu 24.04+), o pip3 pode reclamar de 'externally
managed-environment'. Nesse caso, use: pip3 install matplotlib pandas numpy --break-system-packages 
3.4 — Instalar Git (se ainda não tiver) 
$ sudo apt-get install git -y 
# Instala o Git para baixar o repositório 
3.5 — Verificar instalação 
$ gcc --version 
# Deve mostrar GCC 11+ ou compatível 
$ make --version 
# Deve mostrar GNU Make 4+ 
$ python3 --version 
...
# Baixa todo o projeto do GitHub 
...
README.md            
CHANGELOG.md         
LICENSE              - Código-fonte do driver de kernel (C) - Script de compilação - Benchmark simples (compara read vs mmap) - Benchmark de tempo real (com SCHED_FIFO) - Gera gráficos dos resultados - Documentação principal - Histórico de versões - Licença GPL-2.0 
...
$ make 
# Compila o driver (ezdma_fake.ko) e os benchmarks 
✅ Saída esperada (sucesso) 
make -C /lib/modules/6.8.0-XX-generic/build M=/home/julia/trabalho-so-zero-copy 
modules 
CC [M]    ezdma_fake.o 
...
�
� Warnings esperados (podem ser ignorados): Em alguns kernels, podem aparecer avisos sobre 
'compiler differs' ou 'BTF generation'. São avisos cosméticos. O importante é que o arquivo 
'ezdma_fake.ko' seja gerado. 
�
� Verificar arquivos gerados 
$ ls *.ko benchmark benchmark_rt 
# Confirma que os arquivos foram criados 
Deve mostrar: 
...
$ sudo apt-get install linux-headers-$(uname -r) -y 
# Reinstala os cabeçalhos para o kernel atual 
$ make clean && make 
# Recompila do zero 
⚡ 6. CARREGAR O DRIVER NO KERNEL 
6.1 — Verificar que NÃO está carregado 
$ lsmod | grep ezdma 
# Deve dar saída vazia 
�
� Se aparecer algo: Significa que o driver já está carregado. Descarregue primeiro: sudo rmmod 
ezdma_fake 
6.2 — Carregar o módulo 
$ sudo insmod ezdma_fake.ko 
# Insere o módulo no kernel (vai pedir a senha do sudo) 
...
�
� Documentação completa: Este problema está documentado no item 6 do README.md e no arquivo 
DIAGNOSTICO_DMA_ENGINE.md. 
6.3 — Verificar logs do kernel (PROVA DO DMA REAL!) 
$ sudo dmesg | tail -5 
# Mostra as últimas 5 mensagens do kernel 
Saída esperada: 
...
�
� PONTO-CHAVE DO TRABALHO: A linha 'bus=0x...' contém um endereço de bus de 64 bits emitido 
pelo kernel. Este é o endereço que um controlador DMA físico (NVMe, GPU, placa de rede) usaria para 
acessar a memória diretamente. É a evidência empírica do DMA real implementado. 
✅ 7. VALIDAÇÃO FUNCIONAL 
7.1 — Confirmar que /dev/ezdma foi criado 
$ ls -l /dev/ezdma 
# Deve mostrar o character device com permissão crw-rw-rw- 
Saída esperada: 
crw-rw-rw- 1 root root 234, 0 [data] /dev/ezdma 
ℹ️ Sobre o major number 234: Pode ser diferente em sua máquina (alocação dinâmica). O importante é 
o 'c' inicial (character device) e a permissão 0666 (qualquer usuário pode acessar). 
7.2 — Leitura via read() tradicional 
$ cat /dev/ezdma 
# Lê o conteúdo do device usando read() (caminho tradicional com copy_to_user) 
Saída esperada: 
...
$ ./benchmark 
# Executa 100.000 iterações de cada caminho e compara 
Tempo de execução: ~5 segundos. Saída esperada: 
...
�
� Interpretação: Speedup acima de 10x demonstra empiricamente a eliminação da cópia entre kernel e 
user space. Se houvesse cópia, os tempos seriam similares. 
8.2 — Benchmark de tempo real 
$ sudo ./benchmark_rt 
# Executa com SCHED_FIFO + mlockall + CPU pinning 
Tempo de execução: ~10 segundos. Saída esperada: 
...
Reducao do WCET  : ~38% 
�
� Métricas-chave para tempo real: WCET (Worst Case Execution Time) — pior caso de execução. Jitter 
(desvio padrão) — variabilidade temporal. P99.9 — percentil 99,9% (latência abaixo da qual 99,9% das 
amostras estão). 
8.3 — Geração dos gráficos (opcional) 
$ python3 plot_results.py 
# Gera 3 arquivos PNG a partir dos CSVs salvos pelo benchmark_rt 
Arquivos gerados: 
graph_histogram.png    
graph_cdf.png          
graph_comparison.png   - Histograma de latência - CDF (Cumulative Distribution Function) - Comparativo de métricas (escala log) 
�
...
# Remove o módulo do kernel (libera memória DMA, destrói /dev/ezdma) 
...
$ ls /dev/ezdma 2>&1 
# Deve mostrar erro 'No such file or directory' 
Saída esperada: 
ls: cannot access '/dev/ezdma': No such file or directory 
✅ Ciclo de vida completo: Driver foi carregado → alocou memória DMA → criou device → recebeu 
chamadas → liberou memória → destruiu device → descarregou. Sem leaks ou panics. 
...
sudo rmmod ezdma_fake && \ 
sudo dmesg | tail -3 
⏱️ Tempo total estimado: Em uma VM Azure Standard_D2s_v3: aproximadamente 5-8 minutos do zero 
ao primeiro benchmark. 
�
...
Solução: rodar com menos processos abertos, em VM dedicada, ou bare-metal. Os números 
absolutos podem variar, mas as tendências (mmap mais rápido e previsível) se mantêm. 
────────────────────────────────────────────────── 
Problema 7: Os números são diferentes dos do README 
Causa: variações naturais entre execuções (cache, hypervisor, hardware). 
Solução: o que importa são as TENDÊNCIAS, não valores absolutos. Os CSVs brutos (results_*.csv) e 
o arquivo resultado_rt.txt mostram os resultados originais coletados no ambiente do trabalho. 
...
�
� Se TODOS os 16 itens deram certo: PARABÉNS! O experimento foi reproduzido com sucesso. O 
trabalho funciona em seu ambiente exatamente como descrito na documentação. Os resultados podem 
variar em valores absolutos devido a diferenças de hardware/hipervisor, mas as TENDÊNCIAS são as 
mesmas.
